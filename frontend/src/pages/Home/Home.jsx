...
const Home = () => {
  ...
  return (
    <>
      ...
      <button
        className="w-16 h-16 flex items-center justify-center rounded-2xl bg-[#2B85FF] hover:bg-blue-600 absolute right-[8rem] bottom-10"
        onClick={() => {
          setOpenPdfModal({ isShown: true });
        }}
      >
        <FaFileUpload className="text-[32px] text-white" />
      </button>
      ...
      <Modal
        ...
        className="w-[40%] max-md:w-[60%] max-sm:w-[70%] max-h-3/4 bg-white rounded-md mx-auto mt-14 p-5 overflow-scroll"
      >
        <UploadPdf onClose={() => setOpenPdfModal({ isShown: false })} getAllNotes={getAllNotes}/>
      </Modal>
    </>
  );
};
